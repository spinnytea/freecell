...
// FIXME call moveCursorWithBasicArrows directly
describe('keyboard.moveCursorWithBasicArrows', () => {
	...
	describe('foundation', () => {
		...
		test('going off-top stops', () => {
			const game = new FreeCell({ cursor: { fixture: 'foundation', data: [1] } }).moveCursor('up');
			expect(game.cursor).toEqual({ fixture: 'foundation', data: [1] });
			expect(game.previousAction.text).toBe('cursor stop');
		});
	...
	describe('cascade', () => {
		...
		describe('going off-top', () => {
			...
			describe('size mismatch: cell + foundation < tableau', () => {
				...
				`(
					'col $col',
					({ col, location, action }: { col: number; location: CardLocation; action: string }) => {
